...
const addCode = (config, user, email) => {
    ...
    let code = {
        id: getRandomArbitrary(111111, 999999),
        time: Date.now(),
        user: user
    }
...
const validateCode = (user, code) => {

    let i = DEFAULT_CODES.findIndex(date => date.id == code && date.user == user)
...
const getCode = () => DEFAULT_CODES

const expireCode = (id, time) => {
    let now = Date.now()

    let elapsed = ((time - now ) / 1000) * - 1

    if (elapsed > 60) {
        const value = Math.round(elapsed / 60)
        
        if(value > timeExpire ) {
            let i = DEFAULT_CODES.findIndex(date => date.id = id )

            DEFAULT_CODES.splice(i,1)
        } 
    }
}
